<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Editar Compra</h2>
    </x-slot>

    <div class="p-6">
        <form method="POST" action="{{ route('compras.update', $compra) }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="numero_factura" value="{{ $compra->numero_factura }}" class="w-full border rounded p-2">
            </div>

            <div>
                <label>Fecha de Compra</label>
                <input type="date" name="fecha_compra" value="{{ $compra->fecha_compra }}" class="w-full border rounded p-2" required>
            </div>

            <div>
                <label>Monto Total</label>
                <input type="number" step="0.01" name="monto_total" value="{{ $compra->monto_total }}" class="w-full border rounded p-2" required>
            </div>
